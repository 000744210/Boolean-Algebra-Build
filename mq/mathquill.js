...
var MathBlock = P(MathElement, function(_, super_) {
  ...
  _.chToCmd = function(ch) {
    var cons;
    if (ch.match(/^[a-zA-Z01\+]$/))// allowed character set
     return Letter(ch);
...
OverLineOperator = LatexCmds.overline = LatexCmds.bar = bind(Style, '\\overline', 'span', 'class="mq-non-leaf mq-overline"');
...
var Embed = LatexCmds.embed = P(Symbol, function(_, super_) {
  ...
  _.parser = function() {
	...
	var overLine = LatexCmds.overline;
	var plus = LatexCmds.add;
	var embeddedField = LatexCmds.MathQuillMathField;
	var classable = LatexCmds['class'];
	LatexCmds = {};
	LatexCmds.overline = overLine;
	LatexCmds.add = plus;
	LatexCmds.MathQuillMathField = embeddedField;
	LatexCmds.editable = embeddedField;
	LatexCmds['class'] = classable;
